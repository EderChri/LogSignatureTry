...
def normalize(X_train, X_test):
    ...
    X_train_norm = []
    for i in range(num_dim):
        X_train_norm.append(X_train.iloc[:,[i]].map(lambda s: (s-mean[i]) / (std[i] + 1e-5)).fillna(0))
    X_train_norm = pd.concat(X_train_norm, axis=1)

    X_test_norm = []
    for i in range(num_dim):
        X_test_norm.append(X_test.iloc[:,[i]].map(lambda s: (s-mean[i]) / (std[i] + 1e-5)).fillna(0))
...
def get_data(dataname, adjust_len=None):
    ...
    if min(len_list) == max(len_list):
        if adjust_len:
            X_train = X_train.map(lambda s: get_same_len(s, max_len=adjust_len))
            X_test = X_test.map(lambda s: get_same_len(s, max_len=adjust_len))
        else:
            pass
    else:
        if adjust_len:
        ...
        X_train = X_train.map(lambda s: get_same_len(s, max_len=max_len))
        X_test = X_test.map(lambda s: get_same_len(s, max_len=max_len))
